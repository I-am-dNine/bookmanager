...
public class BookController {
    ...
    public ApiResponseDto<List<Book>> getAllBooks() {
        return ApiResponseDto.success("書籍清單取得成功", bookService.getAllBooks());
    }
    ...
    public ResponseEntity<ApiResponseDto<Book>> getBookById(@PathVariable Long id) {
        return bookService.getBookById(id)
                .map(book -> ResponseEntity.ok(ApiResponseDto.success("查詢成功", book)))
                .orElse(ResponseEntity.status(404).body(ApiResponseDto.error(404, "找不到該書籍")));
    }

    @PreAuthorize("hasAnyRole('ADMIN','STAFF')")
    @PostMapping
    @Operation(summary = "新增書籍", description = "建立一本新書，需提供書名、作者、ISBN、庫存數量")
    public ResponseEntity<ApiResponseDto<Book>> createBook(@RequestBody @Valid Book book) {
        Book created = bookService.createBook(book);
        return ResponseEntity.ok(ApiResponseDto.success("書籍建立成功", created));
    }

    @PreAuthorize("hasAnyRole('ADMIN','STAFF')")
    @PutMapping("/{id}")
    @Operation(summary = "更新書籍", description = "根據 ID 修改書籍的基本資訊")
    public ResponseEntity<ApiResponseDto<Book>> updateBook(@PathVariable Long id, @RequestBody @Valid Book book) {
        Book updated = bookService.updateBook(id, book);
        return ResponseEntity.ok(ApiResponseDto.success("書籍更新成功", updated));
    }
    ...
    public ResponseEntity<ApiResponseDto<Void>> deleteBook(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponseDto.success("書籍已刪除", null));
    }

    @GetMapping("/search")
    @Operation(summary = "搜尋書籍", description = "可透過書名或作者關鍵字進行模糊查詢，支援分頁")
    public ApiResponseDto<Page<Book>> searchBooks(
        ...
        return ApiResponseDto.success("查詢成功，共 " + result.getTotalElements() + " 筆", result);
    }
}
